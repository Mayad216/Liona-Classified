...
function parseCategory(value: string | null): ServiceFilterCategory {
  ...
  const match = SERVICE_CATEGORIES.find((c) => c.key === value);
...
export function ServicesList({
  ...
  const filtered = useMemo(
    ...
    [active, emirate, q, language, cuisine, isTutoringPage, isMealsPage]
  );
    ...
    <div className="bg-slate-50/60 pb-20">
      <section className="border-b border-slate-200/70 bg-gradient-to-b from-white to-slate-50/80 pt-8 pb-10">
        <div className="container">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-base leading-relaxed text-slate-600">
              {isMoversPage
                ...
                : isTutoringPage
                  ...
                  : "Cleaning, meals, maintenance, moving & more — listings from verified personal and business accounts."}
            </p>
            {isTutoringPage && (
              <Link to="/post?mode=service&category=Language%20Tutoring" className="mt-4 inline-block">
            ...
            {isMealsPage && (
              <Link to="/post?mode=service&category=Homemade%20Meals" className="mt-4 inline-block">
                <Button size="sm" variant="outline" className="gap-1.5">
                  <Plus className="h-4 w-4" />
                  ...
                  placeholder={
                    isMoversPage
                      ...
                      : isTutoringPage
                        ...
                        : "Search cleaning, AC, meals, pest control…"
                  }
      ...
      <div className="container pt-10">
          ...
          {popularCategories.map(({ key, label, description, icon: Icon }) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => {
                  if (key === "Language Tutoring") {
                    navigate("/tutoring");
                    return;
                  }
                  if (key === "Homemade Meals") {
                    navigate("/meals");
                    return;
                  }
                  selectCategory(isActive ? "All" : key);
                }}
                className={cn(
        ...
        <div className="mt-6 flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          {SERVICE_CATEGORIES.filter((c) => !c.popular).map(({ key, label, icon: Icon }) => {
            const isActive = active === key;
            const fromPrice = startingPrices[key];
            return (
              <button
          ...
          <div className="mb-8 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-violet-200/80 bg-gradient-to-r from-violet-50 to-brand-50 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Link
                  to="/post?mode=service&category=Language%20Tutoring"
        ...
        {isMealsPage && (
          <div className="mb-8 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-200/80 bg-gradient-to-r from-amber-50 to-orange-50 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Link
                  to="/post?mode=service&category=Homemade%20Meals"
