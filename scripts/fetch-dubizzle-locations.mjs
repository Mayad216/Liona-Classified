/**
 * Fetch Dubizzle neighborhood trees via Playwright (bypasses Incapsula).
 *
 * Usage:
 *   cd backend/workers/auto-apply && npm install
 *   node ../../../scripts/fetch-dubizzle-locations.mjs
 *
...
/** Dubizzle city subdomains → our emirate names */
const EMIRATES = [
...
function flattenNeighborhoods(node, emirate, parentId = null, out = []) {
  ...
  if (Array.isArray(children)) {
    for (const child of children) {
      flattenNeighborhoods(child, emirate, id ? Number(id) : parentId, out);
    }
  }
...
function parseNeighborhoodPayload(payload, emirate) {
  ...
  if (Array.isArray(payload)) {
    return payload.flatMap((n) => flattenNeighborhoods(n, emirate));
  }

  if (Array.isArray(payload.neighborhoods)) {
    return payload.neighborhoods.flatMap((n) => flattenNeighborhoods(n, emirate));
  }

  if (Array.isArray(payload.data)) {
    return payload.data.flatMap((n) => flattenNeighborhoods(n, emirate));
  }

  if (Array.isArray(payload.results)) {
    return payload.results.flatMap((n) => flattenNeighborhoods(n, emirate));
  }

  if (payload.tree) {
    return flattenNeighborhoods(payload.tree, emirate);
  }
...
async function fetchEmirateLocations(page, authHeader, emirate) {
  ...
  for (const url of candidates) {
    ...
    try {
      ...
      // filters endpoint may nest neighborhoods
...
async function main() {
  ...
  console.log("Loading Dubizzle to capture auth token…");
  const authHeader = await captureAuth(page);

  if (!authHeader) {
    console.error("Could not capture Dubizzle auth token. Try running with headless: false.");
    await browser.close();
    process.exit(1);
  }

  console.log("Auth captured. Fetching emirate location trees…");
  ...
  console.log(`\nSaved ${total} neighborhoods to ${OUT}`);
}
